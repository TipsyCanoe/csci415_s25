...
#include <mpi.h>
#include <math.h>
#include "prog6-2.c"         // Cannon's Algorithm
#include "nelson_complete.c" // Nelson's Algorithm
...
int main(int argc, char *argv[])
{
    ...
    {
        if (rank == 0)
            printf("Running Cannon's algorithm...\n");
        if (time_flag)
        ...
        if (time_flag)
        {
            end_time = MPI_Wtime();
            if (rank == 0)
                printf("Cannon's algorithm execution time: %f seconds\n", end_time - start_time);
        }
    }
    ...
    {
        if (rank == 0)
            printf("Running Nelson's algorithm...\n");
        if (time_flag)
            start_time = MPI_Wtime();
        ...
        if (time_flag)
        {
            end_time = MPI_Wtime();
            if (rank == 0)
                printf("Nelson's algorithm execution time: %f seconds\n", end_time - start_time);
        }
    }
